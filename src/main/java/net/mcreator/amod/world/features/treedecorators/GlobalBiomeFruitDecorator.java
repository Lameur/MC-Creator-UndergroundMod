...
public class GlobalBiomeFruitDecorator extends CocoaDecorator {
	public static final GlobalBiomeFruitDecorator INSTANCE = new GlobalBiomeFruitDecorator();
	public static com.mojang.serialization.Codec<GlobalBiomeFruitDecorator> codec;
	public static TreeDecoratorType<?> tdt;
	static {
		codec = com.mojang.serialization.Codec.unit(() -> INSTANCE);
		tdt = new TreeDecoratorType<>(codec);
		tdt.setRegistryName("global_biome_tree_fruit_decorator");
		ForgeRegistries.TREE_DECORATOR_TYPES.register(tdt);
	}

	public GlobalBiomeFruitDecorator() {
		super(0.2f);
	}
				...
				for (Direction direction : Direction.Plane.HORIZONTAL) {
					if (random.nextFloat() <= 0.25F) {
						...
						if (Feature.isAir(level, blockpos)) {
							biConsumer.accept(blockpos, Blocks.MOVING_PISTON.defaultBlockState());
						}
					}
				}
			});
